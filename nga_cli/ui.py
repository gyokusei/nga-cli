...
def parse_nga_content(content: str) -> Text:
    ...
    text = re.sub(r'\[img\]\./(.*?)\[/img\]', r'[link=https://img.nga.178.com/\1](图片链接)[/link]', text, flags=re.IGNORECASE)
...
def display_topics(console: Console, topics_data: Dict[str, Any], page: int) -> Optional[List[Dict]]:
    ...
    for i, topic in enumerate(sorted_topics, 1):
        title_text = escape(topic.get('subject', '无标题'))
        author = escape(topic.get('author', '未知'))
        replies = str(topic.get('replies', 0))
        
        post_timestamp = topic.get('postdate', 0)
        post_time_str = datetime.datetime.fromtimestamp(post_timestamp).strftime('%Y-%m-%d %H:%M') if post_timestamp else '未知时间'
...
def display_topic_details(console: Console, details: Dict[str, Any], page: int, total_pages: int, settings: Dict[str, Any]):
    """显示帖子详情和回复。"""
