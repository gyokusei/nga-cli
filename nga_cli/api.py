...
class NgaClient:
    ...
    def _request(self, endpoint: str, params: Optional[Dict] = None, method: str = 'GET') -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.client.request(method, url, params=params if method == 'GET' else None, data=params if method == 'POST' else None)
            ...
            if "error" in data and data["error"]:
                console.print(Panel(f"[bold red]API 错误:[/bold red] {data['error'][0]}", border_style="red"))
                return None
